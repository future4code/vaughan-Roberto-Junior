...
function CardPerson(props) {
  ...
  const pushMatch = () => {
    ...
    const url = `https://us-central1-missao-newton.cloudfunctions.net/astroMatch/${props.aluno}/choose-person`;
    const body = {
      id: profiles.id,
      choice: true,
    };
  ...
  const alertNoMatch = () => {
    return (
      <div className="alert alert-danger" role="alert">
        {profiles.name} Não deu match com você 💔 Continue tentando
      </div>
    );
  };
  ...
  const alertMatch = () => {
    return (
      <div className="alert alert-info" role="alert">
        Ebaa! Você Deu Match Com {profiles.name} 💏
      </div>
    );
  };
